...
**Web config ownership**: `cfgMu` protects publication of `Server.cfg` by `mutateConfig` and the file watcher. Published configs, including their maps, slices and optional values, are immutable. Each read operation captures one `configSnapshot()` and passes it explicitly to response helpers, including metrics builders. The lock is released before rendering, HTTP/SSE writes, process operations or OAuth calls. Mutations validate and edit the freshly loaded callback config; request snapshots are never written back.

**Replacement and permission batches**: JSON partial server updates call `UpdateServer` before any rename, so changing command/args/URL invalidates the old cache before a rename can move it. Full imports call `Config.Replace`, which copies persistent state, initializes maps and applies the same tolerant compression normalization as loading. Removed and changed servers queue cache deletion; unchanged entries retain tools. Existing queued operations survive replacement, imported private operations do not, and cache I/O remains best-effort after a successful save. `ApplyPermissionChanges` validates a typed batch against the fresh namespace and servers before committing a copied permission slice. Any error leaves the receiver and disk unchanged and reaches the TUI's error toast. Absent-rule removal remains idempotent for existing resources. The web API's `ReplaceToolPermissions` retains full replacement semantics, including order and duplicates, while sharing reference validation.

**JSON mutation errors**: config operations classify missing resources, conflicts and invalid input with identities that survive `%w` wrapping. The API maps them to 404, 409 and 422; unexpected/storage errors are 500 and malformed JSON is 400. Human-readable messages and the `{"error": ...}` response shape are preserved. OAuth/process errors have separate handling.
...
**Relayed interactions** (see "Server-to-client requests"): a call's time spent paused on a relayed elicitation or sampling request is stored separately (`interactionWaitMsSum` per row, `interactionWaitMs` per recent call) and kept out of the latency figures. The per-tool table gains a sortable **Waited** column only when some call in the window waited, and the recent feed notes "+N waiting" beside the duration. Each relayed request's outcome is counted in `interactions` rows keyed by `(date, namespace, server, method, outcome)`; the page shows them in a **Client requests** card — one row per server and request kind, a pill per outcome (the client's answers first, then withdrawals and fallback reasons, unknown outcomes verbatim), and a "not relayed" badge summing the fallbacks. `/api/metrics` carries the same as `interactions` plus per-tool `interactionWaitMs`. Only names and outcomes, never the request or the answer.
